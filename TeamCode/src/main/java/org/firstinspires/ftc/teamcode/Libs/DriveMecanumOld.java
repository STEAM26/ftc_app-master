...
public class DriveMecanumOld {
    ...
    PIDController PID_TURN = new PIDController(0.1,0.0,0.0,false);


    public DriveMecanumOld(HardwareTestPlatform myRobot, LinearOpMode myOpMode, DataLogger myDl) {
        robot = myRobot;
        opMode = myOpMode;
    ...
    public void pivotLeft(double power, double heading) {
        ...
        while (currentZint < heading+initZ && opMode.opModeIsActive()) {
            /**
    ...
    public void pivotRight(double power, double heading) {
        ...
        while (currentZint > initZ-heading && opMode.opModeIsActive()) {
            /**
             * We are pivoting left so reverse power to the left motor
             */
            LR = power;
    ...
    public void pivotPID(double maxPower, int targetHeading) {
        robot.sensorGyro.resetZAxisIntegrator();//Sets current z to 0
        currentZint = robot.sensorGyro.getHeading();

        //float correctedTargetHeading = AngleUnit.DEGREES.normalize(Math.abs(targetHeading % 360));
        float correctedTargetHeading = Math.abs(targetHeading % 360);
        double power;
        double output;
        double error;

        boolean EXIT = false;
        int timer = 1000;  //ms on target before exit
        long timeOut = System.currentTimeMillis() + (long)timer;

        while(!EXIT){
            output = PID_TURN.getOutput(currentZint, correctedTargetHeading);
            error = PID_TURN.getError();
            ...
            //end
            if(robot.sensorGyro.getHeading() == correctedTargetHeading){
            ...
            //telemetry
            opMode.telemetry.addData("Timeout: ", (timeOut - System.currentTimeMillis()));
            opMode.telemetry.addData("Error: ", error);
            opMode.telemetry.update();
        }
    }

    public void pivoter(double maxPower, int targetHeading) { //first to reset z
        procedure = "Pivot";

        //targetHeading controlls rotation, positive is clockwise, negative is counterclockwise.
        //if targetHeading > 359, will rotate full circle

        robot.sensorGyro.resetZAxisIntegrator();//Sets current z to 0
        currentZint = robot.sensorGyro.getHeading();
        //initZ = robot.mrGyro.getIntegratedZValue(); //= 0
        //currentZint = initZ; //= 0

        int correctedHeading = Math.abs(targetHeading % 360);
        double power = 0.05; //starting
        double minPower = 0.2;
        double powerStepPerdegree = (maxPower - minPower)/correctedHeading; //power to gain/error


        if(targetHeading > 0){//right
            while(currentZint < correctedHeading && opMode.opModeIsActive()){
                currentZint = robot.sensorGyro.getHeading();
                double error = Math.abs(targetHeading-currentZint);

                power = maxPower; //for now until p is done
                LR = power;
                LF = power;
                RR = -power;
                RF = -power;
        ...
        if (targetHeading < 0){//left
            if(currentZint == 0 && correctedHeading != 0){currentZint = 360;}
            while(currentZint > correctedHeading && opMode.opModeIsActive()){
                currentZint = robot.sensorGyro.getHeading();
                if(currentZint == 0 && correctedHeading != 0){currentZint = 360;}
                double error = Math.abs(targetHeading-currentZint);
                ...
                if(error < 10) {//for now until p is done
                    power = 0.1; //10%
                }
        ...
        //else correctedHeading = 0 & do nothing
        motorsHalt();
    }
...
//            myCurrentMotorPosition = robot.motorLF.getCurrentPosition();
    ...
    public void translateTime(double timeOut, double power, double heading) {
        ...
        while (opMode.opModeIsActive() && runtime.time() < timeOutTime) {
            ...
            opMode.telemetry.addData("gyro", robot.mrGyro.getHeading());
            opMode.telemetry.update();
